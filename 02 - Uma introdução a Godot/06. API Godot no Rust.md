...
No GDScript, parâmetro de funções podem ter um valor padrão que é utilizado quando um valor não é passado, isto não existe no Rust^[1], então a API fornece uma interface diferente.
