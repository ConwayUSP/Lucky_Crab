...
Já um _packages_ é uma coleção de _crates_ relacionadas que podem ser publicadas e instaladas juntas. Um pacote é definido por um arquivo `Cargo.toml` que especifica as dependências e outras informações sobre o projeto. O pacote pode conter vários arquivos executáveis, mas geralmente apenas uma biblioteca.
...
use std::fs::*; // Importa todos os elementos de io
```
...
// use std::io;
// use std::io::Writer;
use std::io::{self, Writer, Result}; // faça isso
...
mod Restaurante {
    ...
    pub struct Pizza {
        ...
        tamanho: u32, // tamanho e preço não podem ser acessado do lado de fora do modulo Restaurante
        preco: f32
    }
...
Movemos o código dentro do módulo para um arquivo chamada `restaurante.md` e instanciamos nosso módulo na raiz da crate e importamos nossas necessidades:
