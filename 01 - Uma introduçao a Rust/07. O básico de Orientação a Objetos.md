...
Por fim vamos ao exemplo que você provavelmente vai utilizar bastante, o trait `Clone`. Lembra do capítulo de Sistema de Ownership-Borrowing? `Clone` dá aos nossos tipos o método `.clone()`, que faz uma cópia completa do objeto, sem movê-lo (a famosa _deep copy_).
