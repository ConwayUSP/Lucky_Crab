...
Depois de estudar bastante sobre a linguagem, hora de explorar as ferramentas que estão disponíveis para nós. Vamos aprender sobre biblioteca padrão do Rust, a _std_. Nesse capítulo veremos estruturas de dados, ponteiros inteligentes, e módulos para interagir com a máquina.
...
Estamos a um bom tempo utilizando o tipo `String`, mas agora veremos como esse tipo é implementado e outro métodos de uso. Primeiro de tudo, o tipo `String` é equivalente a um tipo `Vec<u8>`, em que todos os bytes são **garantidos** serem caracteres UTF-8 válidos. Ou seja, podemos usar o tipo `String` da mesma forma que usamos o tipo `Vec`, podemos acessar índices, criar _slices_ (no caso `str`) e iterar sobre strings.
...
Para atualizar o valor em um hashmap temos algumas formas de fazer isso:
...
> [!info] Nota:
> O macro `println!` faz o mesmo que a linha acima com as capacidades extras de formatar texto com paramêtros.
...
fn main() -> io::Result<()> {
    io::copy(&mut io::stdin(), &mut io::stdou())?;
    Ok(())
```
...
Não temos como alocar memória infinitamente para nossa estrutura. Então para evitar isso podiamos usar uma referência certo? Sim, mas enfrentariamos alguns problemas de `ownership` ao longo do caminho, assim desejamos **possuir** um ponteiro para valor que armazenamos -- sem contar que uma lista muita grande poderíamos estourar o _stack_, assim é conveniente armazenarmos valores no _heap_ --, aí é que entra o `Box`.
...
`Rc` é um acrônimo para _reference counting_, que é uma forma de gerenciamento de memória que permite que várias referências compartilhem o mesmo valor armazenado em memória. Ele faz isso mantendo uma contagem de referências para o valor armazenado, incrementando a contagem quando um novo valor é criado que aponta para o mesmo valor e decrementando a contagem quando um valor é descartado. Quando a contagem de referências chega a zero, o valor é descartado da memória.
...
> [!info] Nota:
> `Rc<T>` funciona para casos que rodamos código em uma só _thread_. Para um código asíncrono ou _multithread_, usamos `Arc<T>`, que significa _Atomic Reference Counted_, ou seja, um ponteiro que pode ser usado em múltiplas threads de forma segura, e que funciona como um `Rc<T>`.

### Ponteiro `RefCell<T>`

Vejamos como resolver _mutabilidade interna_, é um padrão de design que permite mutar dados através de uma referência imutável utilizando alguns mecanismos e checagens em tempo de execução. Podemos explorar esse conceito através do tipo `RefCell<T>`, que introduz a mutabilidade interna para nosso código.
